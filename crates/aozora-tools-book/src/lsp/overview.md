...
Every client (VS Code extension, Neovim, Helix, Emacs, Zed) hits the
same binary, so capability changes propagate everywhere without
...
the LSP surface via the [custom protocol extensions](extensions.md);
...
conversion that LSP requires for `utf-16`. The buffer is held as a
UTF-8 rope ([`ropey`](https://docs.rs/ropey)), so `utf-8` is the
cheaper encoding internally.
