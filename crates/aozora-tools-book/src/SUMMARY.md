# Summary

[Welcome](welcome.md)

---

# Getting Started

- [Install](getting-started/install.md)
- [LSP quickstart (any editor)](getting-started/lsp.md)
- [Formatter quickstart](getting-started/fmt.md)

# aozora-fmt (formatter)

- [Overview](fmt/overview.md)
- [CLI reference](fmt/cli.md)

# aozora-lsp (language server)

- [Overview](lsp/overview.md)
- [State model](lsp/state-model.md)
- [Standard LSP capabilities](lsp/capabilities.md)
- [Custom protocol extensions](lsp/extensions.md)
  - [aozora/renderHtml](lsp/render-html.md)
  - [aozora/gaijiSpans](lsp/gaiji-spans.md)
  - [Workspace commands](lsp/commands.md)
- [Diagnostics catalogue](lsp/diagnostics.md)

# tree-sitter-aozora

- [Overview](tree-sitter/overview.md)
- [Grammar surface](tree-sitter/grammar.md)
- [Generated parser.c](tree-sitter/generated-parser.md)

# VS Code extension

- [Architecture](vscode/architecture.md)
- [Preview pane](vscode/preview.md)
- [Bundled .vsix release flow](vscode/release.md)

# Performance

- [Profiling with samply](perf/samply.md)
- [Release profile & PGO](perf/pgo.md)

---

# Contributing

- [Development loop](contrib/dev.md)
- [Lint posture](contrib/lints.md)
- [Fuzzing](contrib/fuzzing.md)
- [Release process](contrib/release.md)
- [Sanitizers](contrib/sanitizers.md)
