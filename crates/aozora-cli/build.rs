//! Build script: embed the pinned `aozora` parser rev + tag so the `aozora`
//! binary's `--version` can report which upstream parser is compiled in.
//!
//! Identical to the per-crate build scripts for `aozora-fmt` / `aozora-lsp`:
//! the workspace `Cargo.lock` is the resolved source of truth for the git rev.
